...
func setDefaultPrefix(cfg *option.DaemonConfig, device string, node *LocalNode) {
	if cfg.EnableIPv4 {
		...
		if ipv4range == nil {
			...
			log.WithField(logfields.V4Prefix, node.IPv4AllocCIDR).Debug("Using autogenerated IPv4 allocation range")
		}
	}
	...
	if cfg.EnableIPv6 {
		...
		if ipv6range == nil && ipv4range != nil {
			...
			log.WithField(logfields.V6Prefix, node.IPv6AllocCIDR).Debug("Using autogenerated IPv6 allocation range")
		}
	}
}
