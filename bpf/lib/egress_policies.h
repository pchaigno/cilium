...
srv6_handling(struct __ctx_buff *ctx, struct in6_addr *sid)
{
	...
	union v6addr nat46_prefix __maybe_unused = NAT46_PREFIX;
	void *data, *data_end;
	struct ipv6hdr *ip6;
	union v6addr saddr;
	struct iphdr *ip4;
...
#  ifdef ENABLE_IPV6
	case bpf_htons(ETH_P_IPV6):
		...
		memcpy(&saddr, &ip6->saddr, sizeof(ip6->saddr));
...
#  ifdef ENABLE_IPV4
	case bpf_htons(ETH_P_IP):
		...
		/* TODO: Trying to plug in the pod IPv6 address instead.  */
		/*saddr.s6_addr32[0] = sid->s6_addr32[0];
		saddr.s6_addr32[1] = sid->s6_addr32[1];
		saddr.s6_addr32[2] = sid->s6_addr32[2];
		if (sid->s6_addr32[3] == 0x13)
			saddr.s6_addr32[3] = 0x11;
		else if (sid->s6_addr32[3] == bpf_ntohl(0x13))
			saddr.s6_addr32[3] = bpf_ntohl(0x11);
		else
			saddr.s6_addr32[3] = sid->s6_addr32[3];*/
		BPF_V6(saddr, LXC_IP);
